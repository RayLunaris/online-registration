                ...
                <tbody className="divide-y divide-slate-800/60 font-medium">
                  {filteredSchools.map((school) => (
                    <tr key={school.id} className="hover:bg-slate-900/50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-slate-300">{school.city}</td>
                      <td className="py-3 px-4 text-slate-400">{school.province}</td>
      ...
      {isDialogOpen && editingSchool && (
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
            <div className="relative z-50 w-full max-w-md rounded-2xl border border-slate-800 bg-slate-950 p-6 text-slate-100 shadow-2xl space-y-4">
        ...
        <Dialog open={Boolean(schoolToDelete)} onOpenChange={(open) => !open && setSchoolToDelete(null)}>
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
            <div className="relative z-50 w-full max-w-md rounded-2xl border border-red-900/50 bg-slate-950 p-6 text-slate-100 shadow-2xl space-y-4">
              <h3 className="font-bold text-white text-base">Hapus Sekolah Asal?</h3>
