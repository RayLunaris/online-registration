  ...
  useEffect(() => {
    ...
    const fetchMetadata = () => {
      schoolService.getSchoolProfile().then((profile) => {
        ...
        console.warn('Navbar failed to load school metadata:', err);
      });
    };
      ...
      <header className="sticky top-0 z-50 w-full border-b border-slate-200/80 bg-white/95 backdrop-blur-md transition-all">
        <div className="w-full max-w-[1440px] mx-auto flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <Link to="/" onClick={handleHomeClick} className="flex items-center gap-3 group shrink-0 select-none">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-white border border-slate-200/90 shadow-2xs group-hover:border-teal-500 group-hover:shadow-xs transition-all overflow-hidden p-1">
              <img 
            ...
            <div className="flex flex-col text-left">
              <span className="font-bold text-slate-900 tracking-tight text-sm sm:text-base leading-tight group-hover:text-teal-700 transition-colors">
                {school?.name || 'SMK Negeri 1 Digital Teknologi'}
              </span>
              <span className="text-[11px] text-teal-700 font-medium leading-none mt-0.5">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            ...
            <Link
              ...
              className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                isHomeActive
                  ? 'text-teal-700 font-semibold bg-teal-50/80'
                  : 'text-slate-600 hover:text-teal-700 hover:bg-slate-50/80'
              }`}
            >
              {t('nav.home') || 'Beranda'}
            </Link>
            ...
            >
              ...
              >
                ...
                <ChevronDown className={`h-3.5 w-3.5 transition-transform duration-200 ${dropdownOpen ? 'rotate-180 text-teal-700' : 'text-slate-400'}`} />
              </button>

              {dropdownOpen && (
                <div className="absolute left-0 top-full mt-1.5 w-52 rounded-xl bg-white p-1.5 shadow-lg ring-1 ring-slate-900/5 border border-slate-100 animate-in fade-in-0 zoom-in-95 duration-150 z-50">
                  <Link
                    to="/#ranking"
                  ...
                  >
                    <Trophy className="h-4 w-4 text-teal-600 shrink-0" />
          ...
          {/* [ID|EN] [Cek Status] [Daftar Sekarang] */}
          <div className="hidden sm:flex items-center gap-2.5">
            {/* [ID|EN] Bilingual Switcher */}
            <button
              type="button"
              onClick={toggleLanguage}
              className="flex items-center rounded-lg border border-slate-200/90 bg-slate-50/80 hover:bg-slate-100 p-0.5 text-xs font-mono transition-colors"
            ...
            >
              <span className={`px-2 py-1 rounded-md text-[11px] transition-all ${
                language === 'id' 
                  ? 'bg-white font-bold text-teal-700 shadow-2xs' 
            ...
            <Link
              to="/cek-status"
              className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border border-slate-200/90 hover:border-teal-300 hover:bg-teal-50/40 text-slate-700 hover:text-teal-700 transition-all ${
                isCheckStatusActive ? 'text-teal-700 bg-teal-50/80 border-teal-300' : ''
              }`}
            >
            ...
            {!isRegistrationOpen ? (
              <Button 
                size="sm" 
                disabled 
                className="h-9 px-4 bg-slate-200 hover:bg-slate-200 text-slate-500 text-xs font-semibold rounded-lg cursor-not-allowed shadow-none select-none"
              >
                Pendaftaran Ditutup
              </Button>
            ) : (
          ...
          {/* 4. MOBILE HAMBURGER & [ID|EN] TOGGLE */}
          <div className="flex items-center gap-2 lg:hidden">
            <button
              type="button"
              onClick={toggleLanguage}
              className="flex items-center rounded-lg border border-slate-200 bg-slate-50 p-0.5 text-xs font-mono"
              aria-label="Toggle language"
            >
              <span className={`px-1.5 py-0.5 rounded text-[10px] ${language === 'id' ? 'bg-white font-bold text-teal-700 shadow-2xs' : 'text-slate-500'}`}>ID</span>
              <span className="text-slate-300 text-[10px]">|</span>
              <span className={`px-1.5 py-0.5 rounded text-[10px] ${language === 'en' ? 'bg-white font-bold text-teal-700 shadow-2xs' : 'text-slate-500'}`}>EN</span>
            </button>
          ...
          <div className="lg:hidden border-t border-slate-200/80 bg-white px-4 py-4 space-y-3 shadow-xl animate-in slide-in-from-top-1 duration-150 max-h-[calc(100vh-4rem)] overflow-y-auto">
            <nav className="flex flex-col space-y-1">
              ...
              <Link
                to="/cek-status"
                onClick={() => setMobileMenuOpen(false)}
                className={`px-3 py-2.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-2.5 border-t border-slate-100 ${
                  isCheckStatusActive
                    ? 'text-teal-800 bg-teal-50 font-semibold'
                    : 'text-slate-700 hover:bg-slate-50'
                }`}
            ...
            <div className="pt-2 border-t border-slate-100 flex flex-col gap-2">
              {!isRegistrationOpen ? (
                <Button 
                  disabled 
                  className="w-full bg-slate-200 text-slate-500 font-semibold h-10 rounded-lg text-sm shadow-none cursor-not-allowed select-none"
                >
                  Pendaftaran Ditutup
                </Button>
              ) : (
