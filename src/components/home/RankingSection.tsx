...
export const RankingSection: React.FC = () => {
  ...
  return (
    <section id="ranking" className="py-14 sm:py-20 bg-white border-b border-slate-200/70 scroll-mt-16 relative">
      <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-12">
        
        {/* 1. SECTION HEADER */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-teal-50 text-teal-700 border border-teal-200/70 mb-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/peringkat">
              <Button size="sm" variant="outline" className="h-10 text-xs font-semibold gap-1.5 border-teal-200 text-teal-700 hover:bg-teal-50">
        ...
        <div className="mb-4 flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-50/80 p-3 rounded-xl border border-slate-200/80">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 text-xs bg-white rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 text-slate-900 placeholder:text-slate-400"
            />
          </div>
          <div className="text-xs text-slate-500 flex items-center gap-1.5 w-full sm:w-auto justify-end">
        ...
        <div className="w-full bg-white border border-slate-200/90 rounded-xl overflow-hidden shadow-2xs">
          {isLoading ? (
            <div className="p-8 text-center space-y-2">
              <div className="h-6 w-6 border-2 border-teal-600 border-t-transparent rounded-full animate-spin mx-auto" />
              <span className="text-xs text-slate-500">Memuat data peringkat sementara...</span>
            </div>
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredEntries.map((entry) => {
                    ...
                    return (
                      <tr key={entry.registration_number} className="hover:bg-teal-50/40 transition-colors">
                        {/* Rank Badge */}
                        ...
                        {/* Total */}
                        <td className="px-4 py-3 text-right font-mono font-bold text-teal-700 text-sm">
          ...
          {/* Table Footer Banner */}
          <div className="p-4 bg-slate-50/60 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
            <span className="text-slate-500">
              *Peringkat diperbarui otomatis secara real-time berdasarkan hasil verifikasi nilai rapor dan prestasi.
            </span>
            <Link to="/peringkat">
              <Button size="sm" className="h-8 px-4 bg-teal-600 hover:bg-teal-700 text-white font-semibold text-xs gap-1.5 shadow-xs">
